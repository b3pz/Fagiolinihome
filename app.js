...
generateMenu.onclick=()=>{s.menuBackup=JSON.parse(JSON.stringify(s.menu));let pool=RECIPES.filter(r=>allowed(r[0]));if(pool.length<4)pool=RECIPES;for(let i=0;i<7;i++){let d=new Date();d.setDate(d.getDate()+i);let a=pool[(i*2)%pool.length],b=pool[(i*2+1)%pool.length];s.menu[dateKey(d)]={lunch:a[0],dinner:b[0],lunchAdapt:'Adatta consistenza, sale e pezzi all’età di Caty e Kiko.',dinnerAdapt:'Adatta consistenza, sale e pezzi all’età di Caty e Kiko.'}}save();renderMenu()}
undoMenu.onclick=()=>{if(s.menuBackup){s.menu=s.menuBackup;s.menuBackup=null;save();renderMenu()}}
...
recipeShopping.onclick=()=>{let r=RECIPES.find(x=>x[0]===recipeDialog.dataset.name);if(r)r[1].forEach(n=>{if(!s.shopping.some(x=>x.name.toLowerCase()===n.toLowerCase()&&!x.done))s.shopping.push({id:crypto.randomUUID(),name:n,qty:'',category:'Alimentari',done:false})});recipeDialog.close();save();go('shop')}
...
laundryNext.onclick=()=>{s.laundry.step=Number(s.laundry.step)>=5?0:Number(s.laundry.step)+1;s.laundry.updatedAt=new Date().toISOString();save();renderHouse()};laundryReset.onclick=()=>{s.laundry={step:0,updatedAt:null};save();renderHouse()};addRoutine.onclick=()=>routineDialog.showModal();routineForm.onsubmit=e=>{e.preventDefault();s.routines.push({id:crypto.randomUUID(),name:routineName.value.trim(),emoji:'🏡',everyDays:Number(routineEvery.value),owner:routineOwner.value,lastDone:null});routineForm.reset();routineDialog.close();save();renderHouse()}

shopForm.onsubmit=e=>{e.preventDefault();s.shopping.push({id:crypto.randomUUID(),name:shopName.value.trim(),qty:shopQty.value.trim(),category:shopCategory.value,done:false});shopForm.reset();save();renderShop()}
...
moneyForm.onsubmit=e=>{e.preventDefault();s.expenses.push({id:crypto.randomUUID(),name:moneyName.value.trim(),amount:Number(moneyAmount.value),category:moneyCategory.value,person:null,month:monthKey(),date:dateKey(),recurring:moneyRecurring.checked});moneyForm.reset();save();renderMoney()}
...
moneyPrev.onclick=()=>{moneyOffset--;renderMoney()};moneyNext.onclick=()=>{if(moneyOffset<0){moneyOffset++;renderMoney()}};addSubscription.onclick=()=>{subDue.value=dateKey();subscriptionDialog.showModal()};subscriptionForm.onsubmit=e=>{e.preventDefault();s.subscriptions.push({id:crypto.randomUUID(),name:subName.value.trim(),amount:Number(subAmount.value),owner:subOwner.value,category:subCategory.value,dueDate:subDue.value,frequency:subFrequency.value,reminderDays:Number(subReminder.value),notify:subNotify.value});subscriptionForm.reset();subscriptionDialog.close();save();renderMoney()}
